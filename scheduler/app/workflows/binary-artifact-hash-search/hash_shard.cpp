...
// Output cap is intentionally large (96 KiB): the worst-case shard reports
// up to ~250 matches as a `index,input,hash;`-delimited string. If this
// were larger we would split the output across more fields or shrink the
// match-density target.
WORKFLOW_NODE_WITH_CAPS(input, output, 64 * 1024, 96 * 1024) {
    ...
    // Browser-side artifact fetch. The workflow JSON declared
    //   "artifacts": [{ "id": "numbers-bin", "file": "numbers.bin" }]
    // and this node opted in with `uses_artifacts: ["numbers-bin"]`. The
    // runtime injects an artifact descriptor with `url`; .fetch_bytes()
    // does the GET via the host fetchx_bytes import. Caps: 16 MB body,
    // 10 s timeout (we pass the max here).
