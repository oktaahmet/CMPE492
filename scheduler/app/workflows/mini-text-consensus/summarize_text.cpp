...
WORKFLOW_NODE(input, output) {
    ...
    // input.node("emit-text") returns the upstream node's FINALIZED output
    // object. The runtime delivers it inline for small outputs and as a
    // chunked stream for large ones — your code reads it the same way.
    // .string("output") pulls the "output" field as a zero-copy string_view
    // into the runtime's input buffer.
